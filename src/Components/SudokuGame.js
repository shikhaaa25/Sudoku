...
import { faUndo, faEraser, faPencil, faInfoCircle, faPause, faPlay } from '@fortawesome/free-solid-svg-icons';
...
const SudokuGame = () => {
    ...
    const updateCellValue = (row, col, value) => {
        ...
        if (!isValidMove(row, col, value)) {
            ...
            setInvalidCells([...invalidCells.filter(cell => cell !== `${row}-${col}`), `${row}-${col}`]);
        } else {
            setInvalidCells(invalidCells.filter(cell => cell !== `${row}-${col}`));
        }
    ...
    }, [selectedCell, board, notes, invalidCells, moveHistory, noteMode, isPaused]);
    ...
    const handleUndo = () => {
        if (moveHistory.length > 0) {
            ...
            setInvalidCells(invalidCells.filter(cell => cell !== `${lastMove.row}-${lastMove.col}`));
        }
    }
        ...
        <>
            <div className={`game-container ${darkMode ? "dark-mode" : ""}`}>
                                ...
                                {row.map((cell, colIndex) => {
                                    ...
                                    return (
                                        <div key={`${rowIndex}-${colIndex}`}
                                            ...
                                            ${initialCells.includes(`${rowIndex}-${colIndex}`) ? 'initial' : ''}
                                            ${invalidCells.includes(`${rowIndex}-${colIndex}`) ? 'invalid' : ''}
                                            ${hint && hint.cell[0] === rowIndex && hint.cell[1] === colIndex ? 'hint' : ''}`}
                ...
                <div className="button-container">
                    <div className="btn-head">
                        ...
                        <span>
                            <FontAwesomeIcon icon={isPaused ? faPlay : faPause} onClick={() => (setIsPaused(!isPaused), setSelectedCell(null))} />
                        </span>
                    </div>
                    <div className="btn-row button_section" >
                        <button className="top-btn-col" onClick={handleUndo} disabled={isPaused}>
                            <FontAwesomeIcon icon={faUndo} />
                        </button>
                        <button className="top-btn-col" onClick={handleErase} disabled={isPaused}>
                            <FontAwesomeIcon icon={faEraser} />
                        </button>
                        <button className="top-btn-col" style={{ position: 'relative' }} onClick={() => setNoteMode(pre => !pre)} disabled={isPaused}>
                            <div className={`${noteMode ? 'hint-count' : 'hint-count-deactive'}`}>
                                {noteMode ? 'ON' : 'OFF'}
                            </div>
                            <FontAwesomeIcon icon={faPencil} />
                        </button>
                        <Tooltip content="Get hint to solve the puzzle">
                        <button className="top-btn-col" style={{ position: 'relative' }} onClick={showHint} disabled={hintCount <= 0 || isPaused}>
                    ...
                    <div className="btn-row">
                        {[1, 2, 3].map((number) => (
                            <button key={number} disabled={isPaused} className="btn-col" onClick={() => handleNumberButtonClick(number)}>
                                {number}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="btn-row">
                        <button className="last-btn-col" disabled={isPaused} onClick={() => startNewGame(difficulty)}>New Game</button>
                    </div>
                </div>
            </div>
            <Modal
            ...
            >
                <>
                    ...
                    <p style={{ fontSize: '20px' }}>You have made 3 mistakes and lost this Game</p>
                    <button className='modal-button' onClick={() => handleStartNewGameAndCloseModal()}>New Game</button>
                </>
            </Modal>
            <Modal
                show={winModal}
            ...
            >
                <>
                    ...
                    <button className='modal-button' onClick={() => handleStartNewGameAndCloseModal()}>Start New Game</button>
                </>
            </Modal>
            ...
            >
                <>
                    ...
                    <button className='modal-button' onClick={() => setErrorModal(false)}>Ok</button>
                </>
            </Modal>
            {hint && (
